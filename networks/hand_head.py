...
class hand_regHead(nn.Module):
    def __init__(self, roi_res=32, joint_nb=21, stacks=1, channels=256, blocks=1):
        ...
        center_offset = 0.5         #偏移量
        vv, uu = torch.meshgrid(torch.arange(self.out_res).float(), torch.arange(self.out_res).float())     #创建网格
        uu, vv = uu + center_offset, vv + center_offset         #添加中心偏移
        self.register_buffer("uu", uu / self.out_res)       #归一化，注册模型缓冲区
        self.register_buffer("vv", vv / self.out_res)       #归一化，注册模型缓冲区
    ...
    def forward(self, x):
        ...
        for i in range(self.stacks):
            ...
            y = self.res[i](y)
            y = self.fc[i](y)
            latents = self.score[i](y)                          # bs x 21 x 32 x 32
            heatmaps= self.spatial_softmax(latents)             # bs x 21 x 32**2 (bs x 21 x 1024) -> bs x 21 x 32 x 32
            out.append(heatmaps)
            predictions = self.generate_output(heatmaps)        # bs x 21 x 2
            ...
            if i < self.stacks - 1:
                ...
                encoding.append(y)                              # bs x 256 x 32 x 32
...
class Bottleneck(nn.Module):            #残差块
    expansion = 2
...
class Hourglass(nn.Module):                                             #沙漏网络
    def __init__(self, block, num_blocks, planes, depth):               #num_blocks->1, planes->128, depth->4

        super(Hourglass, self).__init__()
        self.depth = depth                                                                      #沙漏网络的递归深度
        self.block = block                                                                      #用于构建沙漏网络的残差块
        self.hg = self._make_hour_glass(block, num_blocks, planes, depth)                       #用于递归构建整个沙漏网络的方法

    def _make_residual(self, block, num_blocks, planes):                #创建一个由多个残差快顺序堆叠的模块
    ...
    def _make_hour_glass(self, block, num_blocks, planes, depth):       #该方法 构建一个长度为depth的列表hg  hg[i]对应沙漏中第i+1阶所需要的所有模块
        hg = []
    ...
    def _hour_glass_forward(self, n, x):                                #该方法 沙漏网络的前向传播逻辑  递归方式实现
