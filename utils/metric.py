...
def eval_batch_obj(batch_output, obj_bbox,
    ...
    for i in range(bs):
        ...
        bbox = obj_bbox[i]      #当前边界框
        width, height = bbox[2] - bbox[0], bbox[3] - bbox[1]        #宽、高
        cord_upleft = [bbox[0], bbox[1]]        #左上角坐标
        intrinsics = cam_intr[i]        #相机内参
        bbox_3d = obj_bbox3d[i]         #3D边界框
        if torch.is_tensor(obj_cls[i]):         #obj_cls[i]是当前物体的类别，应该是一个序号，一个序号表示一类物体
            cls = int(obj_cls[i])               #转换为整数实例
        else:
            cls = obj_cls[i]
        mesh = mesh_dict[cls]                   #获取对应类别物体的3D网格
        hand_type=batch_hand_type[i]
        if batch_affinetrans is not None:
            affinetrans = batch_affinetrans[i]      #仿射变换参数
        else:
            affinetrans = None
        pred_pose, p2d = fuse_test(output, width, height, intrinsics, bestCnt, bbox_3d, cord_upleft,
                                   affinetrans=affinetrans,hand_type=hand_type)                             #pred_pose是物体的6D姿态
