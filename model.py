...
class HONet(nn.Module):
    def __init__(self, roi_res=32, joint_nb=21, stacks=1, channels=256, blocks=1,
        ...
        self.transformer_hand = Transformer(inp_res=roi_res, dim=channels,depth=transformer_depth, num_heads=transformer_head)
    ...
    def net_forward(self, imgs, bbox_hand, bbox_obj,mano_params=None, roots3d=None):
        ...
        # P2 from FPN Network
        P2_h,P2_o = self.base_net(imgs)     #提取手和物体特征       bs x 256 x 64 x 64
        idx_tensor = torch.arange(batch, device=imgs.device).float().view(-1, 1)
        # get roi boxes
        roi_boxes_hand = torch.cat((idx_tensor, bbox_hand), dim=1)      #手边界框
        roi_boxes_inter = torch.cat((idx_tensor, bbox_inter), dim=1)  # 手和物体相交区域边界框
        ...
        x_hand = ops.roi_align(P2_h, roi_boxes_hand, output_size=(self.out_res, self.out_res), spatial_scale=1.0/4.0,
                          sampling_ratio=-1)  # hand        #手感兴趣区域特征   bs x 256 x 32 x 32

        x_obj = ops.roi_align(P2_o, roi_boxes_hand, output_size=(self.out_res, self.out_res), spatial_scale=1.0/4.0,
                          sampling_ratio=-1)  # hand        #物体中的手区域特征   bs x 256 x 32 x 32

        # x_inter = ops.roi_align(P2_o, roi_boxes_inter, output_size=(self.out_res, self.out_res), spatial_scale=1.0/4.0,
        #                   sampling_ratio=-1)  # hand        #物体中与手相关的特征   bs x 256 x 32 x 32
        # x_inter = msk_inter[:, None, None, None] * x_inter

        # obj forward
        if self.reg_object:
            roi_boxes_obj = torch.cat((idx_tensor, bbox_obj), dim=1)        #物体边界框
            # roi_boxes_inter = torch.cat((idx_tensor, bbox_inter), dim=1)        #手和物体相交区域边界框

            y = ops.roi_align(P2_o, roi_boxes_obj, output_size=(self.out_res, self.out_res), spatial_scale=1.0 / 4.0,
                              sampling_ratio=-1)  # obj        #物体感兴趣区域特征       bs x 256 x 32 x 32



            # z_x = ops.roi_align(P2_h, roi_boxes_inter, output_size=(self.out_res, self.out_res), spatial_scale=1.0 / 4.0,
            #                   sampling_ratio=-1)  # intersection        #手中与物体相关的特征       bs x 256 x 32 x 32


            # z_x = msk_inter[:, None, None, None] * z_x      #类似与一个掩码操作，将存在手物相交情况保留交互区域特征，将不存在相交情况的特征去掉

            #print(3)

            # hand_obj = torch.cat([x_hand,x_obj.detach()],dim=1)     #按通道维度拼接了手相关特征
            hand = self.transformer_hand(x_hand, x_obj.detach())         #obj-to-hand enhanced   自、交叉注意

            # y = self.transformer_obj(y, z_x.detach())       #hand-to-obj enhanced   跨注意

            out_fm = self.obj_head(y)       #物体回归头
            preds_obj = self.obj_reorgLayer(out_fm)     #物体2D姿态解码
        else:
            preds_obj = None
        ...
        out_hm, encoding, preds_joints = self.hand_head(hand)       #手回归头  out_hm->热图, encoding->编码特征, preds_joints->坐标预测(2d)

        mano_encoding = self.hand_encoder(out_hm, encoding)         #手编码器  将热图和特征编码为一个紧凑的特征向量

        pred_mano_results, gt_mano_results = self.mano_branch(mano_encoding, mano_params=mano_params, roots3d=roots3d)          #MANO回归头  从特征向量回归MANO手部模型的姿态和形状参数，并生成3D网格和关节点坐标

        return preds_joints, pred_mano_results, gt_mano_results, preds_obj      #pred_mano_results->包含mano参数和预测3D坐标的字典
    ...
    def forward(self, imgs, bbox_hand, bbox_obj,
        ...
        if self.training:
            ...
            if mano_params is not None:
                mano_total_loss, mano_losses = self.mano_loss.compute_loss(pred_mano_results, gt_mano_results)      #MANO姿态参数、形状参数、预测的3D关节点、网格点的损失计算  MSE/L2
                total_loss += mano_total_loss
                for key, val in mano_losses.items():
                    losses[key] = val
            if joints_uv is not None:
                joint2d_loss, joint2d_losses = self.joint2d_loss.compute_loss(preds_joints2d, joints_uv)            #预测的2D关节点的损失计算  MSE/L2
                for key, val in joint2d_losses.items():
                    losses[key] = val
                total_loss += joint2d_loss
            if preds_obj is not None:
                obj_total_loss, obj_losses = self.object_loss.compute_loss(obj_p2d_gt, obj_mask, preds_obj, obj_lossmask=obj_lossmask)      #物体2D坐标回归损失、置信度损失   L1
                for key, val in obj_losses.items():
                    losses[key] = val
                total_loss += obj_total_loss
            if total_loss is not None:
                losses["total_loss"] = total_loss.detach().cpu()        #总损失
            else:
                losses["total_loss"] = 0
