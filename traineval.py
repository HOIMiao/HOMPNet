...
def main(args):
    ...
    if not args.evaluate:
        ...
        if args.pretrained_model !='':
            model.load_state_dict(torch.load(args.pretrained_model, map_location='cuda')['state_dict'])
            optimizer.param_groups[0]["lr"] = torch.load(args.pretrained_model, map_location='cuda')['current_lr']
            start_epoch = torch.load(args.pretrained_model, map_location='cuda')['epoch']
